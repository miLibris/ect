...
(function () {
	...
	var	ECT = function (options) {
		...
		TemplateContext.prototype.load = function (template) {
			var file, compiled, container, data, filePath, fileStat;
			...
			if (ect.options.cache && cache[template]) {
				...
				var makePath = function (root) {
					if (typeof process !== 'undefined' && process.platform === 'win32') {
						return path.normalize((root.length && template.charAt(0) !== '/' && template.charAt(0) !== '\\' && !/^[a-zA-Z]:/.test(template) ? (root + '/') : '') + template.replace(extExp, '') + ect.options.ext);
					} else {
						return path.normalize((root.length && template.charAt(0) !== '/' ? (root + '/') : '') + template.replace(extExp, '') + ect.options.ext);
					}
				};
				if (Object.prototype.toString.call(ect.options.root) === '[object String]') {
					file = makePath(ect.options.root);
				} else if (Object.prototype.toString.call(ect.options.root) === '[object Array]') {
					for (var i = 0; i < ect.options.root.length; ++i) {
						filePath = makePath(ect.options.root[i]);
